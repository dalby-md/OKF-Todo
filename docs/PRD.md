...
- Treat MCP as a broad secondary or primary task interface, not a narrow import adapter. Expose the ordinary task domain through discoverable read and write tools: filtered task search, complete task context, safe partial edits, lifecycle actions, comments, checklists, relationships, attachments, reversible Trash, and task-list organization. Keep database replacement/reset, permanent task deletion, lookup administration, sample-data maintenance, and UI preferences in the desktop application.
- Maintain MCP applicability as a standing product rule as the application evolves. Every new or changed capability must be assessed in the same change and exposed through MCP when it has a useful, safe headless workflow; exclusions must have a concrete documented reason and must never happen by omission.
...
The preview shows up to three checklist items per task initially and provides an explicit **Show all N** / **Show fewer** control when more exist. The exported Markdown and HTML always contain every checklist item; preview collapsing never removes content from the export. Checklist-item sorting uses the same completion-ratio and item-count order as checklist progress.

Multiline text and Markdown table delimiters are escaped so the generated table remains valid. Bodies, comments, Timeline entries, relationships, and attachment content are not included. Checklist item text and current completion state are included only when **Checklist items** is selected. This remains an operational work inventory, not an archival export.
...
Trash is never included in Markdown task export, and the Export action is not available in the Trash view. A database backup remains the only supported restorable copy of task bodies, attachments, relationships, comments, complete checklist records, history, and trashed tasks.
