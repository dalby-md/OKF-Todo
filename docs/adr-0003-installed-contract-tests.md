...
- `{install}\Okf-Todo.exe`, invoked either with `--okf-command` and `--okf-database-path` as directed by the installed OKF application-command reference, or with `--mcp` and `--database-path` for MCP stdio.
...
The tests must not reference `Okf-Todo` or another application project. They must not read the repository's `docs\okf` directory, installer staging directory, publish output, application database, or user preferences.

The installed executable's OKF command and MCP modes are hard preconditions. A missing `{install}\Okf-Todo.exe` fails environment validation and does not skip tests.
...
- **MCP path:** invoke the relevant MCP tools through `{install}\Okf-Todo.exe --mcp --database-path <temporary-database>`.
...
1. Installation contract: the required installed executable and OKF entry point exist.
...
- The tests exercise the same installed executable modes available to a user and harness.
