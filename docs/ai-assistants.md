...
An assistant working directly with OKF Todo first reads the installed
[OKF instructions](what-is-okf.md), then uses the active SQLite database. OKF
...
OKF Todo keeps its data in one local SQLite database. The assistant can read
your real task data instead of relying on text that you copy into the
conversation.

With access to the database, the assistant can:
...
Before working with the database, the assistant should first read the installed
[OKF files](what-is-okf.md), which explain the database structure and OKF
Todo's rules.
...
When considering another product, ask two simple questions:

1. Can it read files from locations I approve?
2. Can it ask before making changes and show me the saved result afterward?
