...
`docs/help/using-okf-todo.md`, `docs/help/okf-layer.md`, and `docs/help/mcp-server.md` are end-user guides and are the canonical source for the application's offline Help.

- Write `docs/help/using-okf-todo.md` for a person using the desktop application to capture, organize, find, update, complete, recover, and protect work. Explain behavior from the user's perspective, follow the order in which users encounter the interface, use exact current UI labels, and keep implementation details out of the main workflow.
- **Strict product-guide maintenance rule:** every change to user-visible behavior covered by `docs/help/using-okf-todo.md` must update that guide in the same change. This includes changes to task creation, saving, fields, editor behavior, lists, views, search, filters, sorting, selection, lifecycle actions, Trash, preferences, backup, attachments, relationships, comments, Timeline, labels, shortcuts, or navigation. Treat a feature change without the corresponding guide update as incomplete.
- Write `docs/help/okf-layer.md` and `docs/help/mcp-server.md` for a user who wants to use a harness such as Codex or Claude Code to turn customer email, support transcripts, notes, logs, and similar source material into useful artifacts.
- In those two integration guides, lead with outcomes, practical workflows, prompt examples, review and approval steps, privacy guidance, and current product boundaries.
...
- When changing related OKF or MCP implementation details, update the applicable integration guide without changing its end-user target.
- **Strict synchronization rule:** every change to any canonical Help Markdown file must update the in-program offline Help in the same change. These canonical files must be copied unchanged into build and publish output; never maintain a separately edited Help copy under `wwwroot`.
- After changing any canonical Help file, build the desktop project and verify that the corresponding file under the normal application output `wwwroot/help` directory exactly matches the canonical source. Treat a documentation-only diff without this verification as incomplete. A temporary or alternate validation output does not count as updating the program Help; refresh the normal application output and any publish or installer artifact included in the task.
